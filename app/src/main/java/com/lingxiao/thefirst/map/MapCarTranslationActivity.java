...
public class MapCarTranslationActivity extends BaseActivity {
    @BindView(R.id.map)
    MapView mMapView;
    ...
    public int getLayoutResource() {
        return R.layout.activity_map_car_translation;
    }
...
//        textureList.add(BitmapDescriptorFactory.fromResource(R.drawable.ic_custtexture));
...
//                .setCustomTexture(BitmapDescriptorFactory.fromResource(R.drawable.ic_custtexture))
//                .addAll(list)
//                .useGradient(true)
//                .width(18));
...
//        mAMap.animateCamera(CameraUpdateFactory.newLatLngBounds(builder.build(), 100));
    ...
    /**
     * 思路
     * 先将坐标点数组转变成经纬度点坐标集合，根据经纬度坐标点集合创建轨迹路线
     */
    // 开始移动
    public void startMove() {
        ...
        // 创建轨迹路线
        mPolyline = mAMap.addPolyline(new PolylineOptions()
                // 默认是一条黑色线，此处是用这个图标链接起来形成一条线路
                .setCustomTexture(BitmapDescriptorFactory.fromResource(R.drawable.ic_custtexture))
                .addAll(points)
                .useGradient(true)// 获取线段是否使用渐变色，注：肉眼看区别不大
                .width(18));// 轨迹跨度
        ...
        // 设置 平滑移动的 图标
        Marker marker = mAMap.addMarker(new MarkerOptions().icon(BitmapDescriptorFactory.fromResource(R.drawable.icon_car)).anchor(0.5f, 0.5f));
        // 实例 MovingPointOverlay 对象
        MovingPointOverlay smoothMarker = new MovingPointOverlay(mAMap, marker);
